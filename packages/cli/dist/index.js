...
`}async function ie(e,n){let t=process.cwd();switch(e){case"saas":await Me(t,n);break;case"api":await ze(t,n);break;case"usage":await Ke(t,n);break;case"minimal":break;default:throw new Error(`Unknown template: ${e}`)}}async function Me(e,n){await L("all",{path:"components/billing"});let t=n.find(o=>o.name==="Pro"),r=n.find(o=>o.name==="Enterprise"),s=t?.priceId||"price_placeholder_pro",a=r?.priceId||"price_placeholder_enterprise",m=`"use client";
...
`;await k.ensureDir(S.join(e,"app/pricing")),await k.writeFile(S.join(e,"app/pricing/page.tsx"),m);let u=`"use client";
...
`;await k.ensureDir(S.join(e,"app/billing")),await k.writeFile(S.join(e,"app/billing/page.tsx"),u),await k.writeFile(S.join(e,"app/demo/page.tsx"),`"use client";
...
`));let o={projectScaffolded:r||!s,dependencies:!1,stripeProducts:!1,database:!1,templates:!1,env:!1},h=[],x=T("Installing dependencies...").start();try{await xe(["@drew/billing-sdk","stripe"],a,x),x.succeed("Dependencies installed"),o.dependencies=!0}catch(c){x.fail("Failed to install dependencies");let d=c instanceof Error?c.message:String(c);h.push(`Dependencies: ${d}`),console.log(i.gray(`Run manually: ${a} ${a==="npm"?"install":"add"} @drew/billing-sdk stripe`))}if(o.dependencies){let c=T("Installing additional dependencies...").start();try{await xe(["drizzle-orm","@neondatabase/serverless","drizzle-kit","@types/node","typescript","stripe"],a,c,!0),c.succeed("Additional dependencies installed")}catch{c.warn("Some additional dependencies may need manual installation"),console.log(i.gray("You can install them later if needed."))}}let f=[];if(p.createProducts&&p.stripeSecretKey){let c=T("Creating Stripe products...").start();try{if(!p.stripeSecretKey.startsWith("sk_test_")&&!p.stripeSecretKey.startsWith("sk_live_"))throw new Error("Invalid Stripe secret key format");f=await se(p.stripeSecretKey),c.succeed(`Created ${f.length} Stripe products`),o.stripeProducts=!0}catch(d){c.fail("Failed to create Stripe products");let W=d instanceof Error?d.message:String(d);h.push(`Stripe products: ${W}`),console.log(i.gray("You can create them manually in the Stripe Dashboard")),console.log(i.gray("Then update the price IDs in your code")),f=[{id:"prod_fallback",name:"Pro",priceId:"price_fallback_pro"},{id:"prod_fallback_2",name:"Enterprise",priceId:"price_fallback_enterprise"}]}}let w=T("Setting up database...").start();try{await et(t),await tt(t,a,w),w.succeed("Database configured"),o.database=!0}catch(c){w.fail("Database setup failed");let d=c instanceof Error?c.message:String(c);h.push(`Database: ${d}`),console.log(i.gray("You can set up the database later by running:")),console.log(i.gray("  npx drizzle-kit push")),console.log(i.gray(`
Make sure to set DATABASE_URL in your .env.local file`))}let P=T(`Installing ${p.template} template...`).start();try{await C.ensureDir(N.join(t,"app")),await C.ensureDir(N.join(t,"components")),await ie(p.template,f),P.succeed("Template installed"),o.templates=!0}catch(c){P.fail("Template installation failed");let d=c instanceof Error?c.message:String(c);h.push(`Templates: ${d}`),console.log(i.gray("Try running:")),console.log(i.gray("  npx @drew/billing add all"))}let re=T("Updating environment variables...").start();try{let c={STRIPE_SECRET_KEY:p.stripeSecretKey,NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY:p.stripePublishableKey,STRIPE_WEBHOOK_SECRET:p.webhookSecret||"whsec_... (run: stripe listen --forward-to localhost:3000/api/webhooks/stripe)",BILLING_API_URL:"http://localhost:3000"},d=N.join(t,".env.local");(await C.readFile(d,"utf-8").catch(()=>"")).includes("DATABASE_URL=")||(c.DATABASE_URL="postgresql://username:password@localhost:5432/database_name"),await ce(c),re.succeed("Environment variables configured"),o.env=!0}catch(c){re.fail("Failed to update .env");let d=c instanceof Error?c.message:String(c);h.push(`Environment: ${d}`)}let J=Date.now()-n;if(te(ee.INIT_COMPLETED,{template:p.template,durationMs:J,framework:u.name,success:Object.values(o).every(c=>c)}),ye("init_complete",J),console.log(i.green.bold(`
\u2705 Setup complete!
`)),h.length>0&&(console.log(i.yellow("\u26A0\uFE0F  Some steps failed:")),h.forEach(c=>console.log(i.gray(`  \u2022 ${c}`))),console.log()),console.log(i.white(`Next steps:
`)),o.projectScaffolded?(console.log(i.gray("1."),"Navigate to your project:",i.cyan(`cd ${m}`)),console.log(i.gray("2."),"Start your dev server:",i.cyan(`${a==="npm"?"npm run":a} dev`)),console.log(i.gray("3."),"Start Stripe webhook listener:",i.cyan("stripe listen --forward-to http://localhost:3000/api/stripe/webhook"))):(console.log(i.gray("1."),"Start your dev server:",i.cyan(`${a==="npm"?"npm run":a} dev`)),console.log(i.gray("2."),"Start Stripe webhook listener:",i.cyan("stripe listen --forward-to http://localhost:3000/api/stripe/webhook"))),o.templates){let c=o.projectScaffolded?"4":"3";console.log(i.gray(`${c}.`),"Visit",i.cyan("http://localhost:3000/pricing"))}o.database||(console.log(i.gray(`
\u26A0\uFE0F  Database not configured. Add DATABASE_URL to .env.local and run:`)),console.log(i.gray("   npx drizzle-kit push"))),console.log(),console.log(i.gray("Documentation:"),i.underline("https://billing.drew.dev/docs")),console.log(i.gray("Diagnostics:"),i.cyan("npx @drew/billing doctor")),console.log(i.gray("Support:"),i.underline("https://github.com/drew/billing/issues")),console.log(),f.length>0&&o.stripeProducts?(console.log(i.gray("Created Stripe products:")),f.forEach(c=>{console.log(i.gray(`  \u2022 ${c.name}: ${c.priceId}`))}),console.log()):f.length>0&&(console.log(i.gray("Placeholder product IDs (update these in your code):")),f.forEach(c=>{console.log(i.gray(`  \u2022 ${c.name}: ${c.priceId}`))}),console.log()),console.log(i.blue("\u{1F4CA} Help improve @drew/billing")),console.log(i.gray("Enable anonymous telemetry to help us fix bugs faster.")),console.log(i.gray(`Run: npx @drew/billing telemetry --enable
`)),await ve("init_completed",{template:p.template,framework:u.name,durationMs:J,results:o})}async function Qe(e){try{return(await C.readdir(e)).filter(r=>!r.startsWith(".")&&r!=="node_modules").length===0}catch{return!0}}async function Ze(e,n=!1){let t=N.basename(e),r=N.dirname(e),s="npm";try{await A("bun",["--version"],{stdio:"pipe"}),s="bun"}catch{try{await A("pnpm",["--version"],{stdio:"pipe"}),s="pnpm"}catch{try{await A("yarn",["--version"],{stdio:"pipe"}),s="yarn"}catch{}}}let a=T(`Creating Next.js project with ${s}...`).start();try{let m=s==="npm"?"npx":s,u=[...s==="npm"?["create-next-app@latest"]:["create","next-app"],t,"--typescript","--tailwind","--eslint","--app","--src-dir=false","--import-alias","@/*",...n?["--yes"]:[]];return await A(m,u,{cwd:r,stdio:"pipe",timeout:3e5}),a.succeed("Next.js project created"),{success:!0,pkgManager:s,projectName:t}}catch{if(a.fail("Failed to create Next.js project"),s!=="npm"){a.text="Retrying with npm...",a.start();try{return await A("npx",["create-next-app@latest",t,"--typescript","--tailwind","--eslint","--app","--src-dir=false","--import-alias","@/*",...n?["--yes"]:[]],{cwd:r,stdio:"pipe",timeout:3e5}),a.succeed("Next.js project created with npm"),{success:!0,pkgManager:"npm",projectName:t}}catch{return a.fail("All attempts failed"),{success:!1,pkgManager:"npm",projectName:t}}}return{success:!1,pkgManager:s,projectName:t}}}async function xe(e,n,t,r=!1,s=2){let a=n==="npm"?"install":"add",m=r?n==="npm"?"--save-dev":"-D":"",u=[a,...e,...m?[m]:[]];for(let p=1;p<=s;p++)try{t.text=`Installing dependencies (attempt ${p}/${s})...`,await A(n,u,{cwd:process.cwd(),stdio:"pipe",timeout:12e4});return}catch(o){if(p===s)throw o;await new Promise(h=>setTimeout(h,2e3))}}async function et(e){let n=N.join(e,"drizzle.config.ts");if(await C.pathExists(n)||await C.pathExists(N.join(e,"drizzle.config.js")))return;await C.writeFile(n,`import { defineConfig } from "drizzle-kit";
...
`);let r=N.join(e,"drizzle");await C.ensureDir(r),await C.writeFile(N.join(r,"schema.ts"),`import { pgTable, serial, varchar, timestamp, integer, jsonb } from "drizzle-orm/pg-core";
...
`));let e=[],n=z("Checking environment variables...").start();try{let o=M.join(process.cwd(),".env.local");if(!await O.pathExists(o))e.push({name:"Environment File",status:"fail",message:".env.local not found"}),n.fail();else{let x=await O.readFile(o,"utf-8"),w=["STRIPE_SECRET_KEY","NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY"].filter(P=>!x.includes(P));w.length>0?(e.push({name:"Environment Variables",status:"fail",message:`Missing: ${w.join(", ")}`}),n.fail()):(e.push({name:"Environment Variables",status:"pass",message:"All required variables present"}),n.succeed())}}catch{e.push({name:"Environment Variables",status:"fail",message:"Could not read .env file"}),n.fail()}let t=z("Checking Stripe connection...").start();try{let o=(await import("stripe")).default,x=await new o(process.env.STRIPE_SECRET_KEY,{apiVersion:"2024-12-18.acacia"}).accounts.retrieve();e.push({name:"Stripe API",status:"pass",message:`Connected to ${x.settings?.dashboard?.display_name||"Stripe account"}`}),t.succeed()}catch{e.push({name:"Stripe API",status:"fail",message:"Could not connect to Stripe API"}),t.fail()}let r=z("Checking database...").start();try{let o=await O.pathExists(M.join(process.cwd(),"drizzle.config.ts")),h=await O.pathExists(M.join(process.cwd(),"drizzle/schema.ts"));o&&h?(e.push({name:"Database Setup",status:"pass",message:"Drizzle ORM configured"}),r.succeed()):(e.push({name:"Database Setup",status:"warn",message:"Database configuration not detected"}),r.warn())}catch{e.push({name:"Database Setup",status:"warn",message:"Could not verify database setup"}),r.warn()}let s=z("Checking API routes...").start();try{let o=["api/checkout/route.ts","api/webhooks/stripe/route.ts","api/entitlements/[userId]/route.ts"],h=M.join(process.cwd(),"app"),x=[];for(let f of o){let w=M.join(h,f);await O.pathExists(w)||x.push(f)}x.length>0?(e.push({name:"API Routes",status:"warn",message:`Missing routes: ${x.length}`}),s.warn()):(e.push({name:"API Routes",status:"pass",message:"All required routes present"}),s.succeed())}catch{e.push({name:"API Routes",status:"warn",message:"Could not verify API routes"}),s.warn()}let a=z("Checking SDK...").start();try{let o=await O.readJson(M.join(process.cwd(),"package.json"));o.dependencies?.["@drew/billing-sdk"]||o.devDependencies?.["@drew/billing-sdk"]?(e.push({name:"SDK Installation",status:"pass",message:"@drew/billing-sdk installed"}),a.succeed()):(e.push({name:"SDK Installation",status:"fail",message:"SDK not found in dependencies"}),a.fail())}catch{e.push({name:"SDK Installation",status:"fail",message:"Could not check package.json"}),a.fail()}console.log(b.blue.bold(`
\u{1F4CA} Summary
`));let m=e.filter(o=>o.status==="pass").length,u=e.filter(o=>o.status==="fail").length,p=e.filter(o=>o.status==="warn").length;e.forEach(o=>{let h=o.status==="pass"?b.green("\u2713"):o.status==="fail"?b.red("\u2717"):b.yellow("\u26A0"),x=o.status==="pass"?b.green:o.status==="fail"?b.red:b.yellow;console.log(`${h} ${x(o.name)}`),console.log(b.gray(`  ${o.message}`))}),console.log(),u===0?(console.log(b.green.bold("\u2705 All checks passed!")),console.log(b.gray("Your billing setup looks good."))):u>0&&m>0?(console.log(b.yellow.bold("\u26A0\uFE0F  Some checks failed")),console.log(b.gray("Review the issues above to complete your setup."))):(console.log(b.red.bold("\u274C Setup incomplete")),console.log(b.gray("Run: npx @drew/billing init"))),console.log(),console.log(b.gray("Next steps:")),console.log(b.gray("  \u2022 Start dev server: npm run dev")),console.log(b.gray("  \u2022 Start webhook listener: stripe listen --forward-to localhost:3000/api/webhooks/stripe")),console.log(b.gray("  \u2022 View docs: https://billing.drew.dev/docs")),console.log()}import v from"chalk";import nt from"ora";import Pe from"fs-extra";import rt from"path";async function Ie(e){console.log(v.blue.bold(`
\u{1F3D6}\uFE0F  @drew/billing sandbox
`));let n=rt.join(process.cwd(),".env.local"),t="";try{t=await Pe.readFile(n,"utf-8")}catch{}let r;if(e.enable)r=!0;else if(e.disable)r=!1;else{let a=t.match(/BILLING_SANDBOX_MODE=(true|false)/);r=!(a?a[1]==="true":!1)}let s=nt(r?"Enabling sandbox mode...":"Disabling sandbox mode...").start();try{t.includes("BILLING_SANDBOX_MODE=")?t=t.replace(/BILLING_SANDBOX_MODE=(true|false)/,`BILLING_SANDBOX_MODE=${r}`):t+=`
# Sandbox mode - no real charges
BILLING_SANDBOX_MODE=${r}
`,await Pe.writeFile(n,t),s.succeed()}catch(a){s.fail("Failed to update sandbox mode"),console.log(a),process.exit(1)}r?(console.log(v.green.bold(`
...
`)),n.optedInAt||(console.log(g.blue("\u{1F4A1} Why enable telemetry?")),console.log(g.gray("Anonymous data helps us improve the CLI and catch bugs faster.")),console.log(g.gray(`No personal information is ever collected.
