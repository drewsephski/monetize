...
function PricingContent() {
  ...
  const preferredPlan = useMemo(() => (searchParams.get("intent") === "checkout" ? "pro" : null), [searchParams]);
  ...
  return (
    <SiteShell site={siteConfig} activePath="/pricing">
      <PageSection className="py-14 md:py-18">
        <SectionHeading
          eyebrow="Pricing"
          title="Billing language that speaks to API buyers."
          description="Each plan focuses on concrete numbers a developer can reason about quickly: calls, request rate, key count, and analytics depth."
        />
      </PageSection>
      ...
      <PageSection className="py-10">
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
          ...
          <Surface className="p-6" strong>
            <div className="signal-pill">First-run guidance</div>
            <p className="mt-5 text-sm leading-7 text-[color:var(--ink-2)]">
              Trigger a Pro checkout first. The dashboard will send you straight to API key creation, because that is the next task an API buyer actually needs to do.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <ButtonLink href="/api-keys" variant="secondary">
                Inspect API Keys
              </ButtonLink>
              <ButtonLink href="/usage" variant="ghost">
                Inspect Usage
              </ButtonLink>
            </div>
          </Surface>
